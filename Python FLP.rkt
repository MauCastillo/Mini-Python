...
(define grammar-simple-interpreter
  '((program (expression) a-program)
    ...
    (expression("def" identifier "(" (separated-list identifier "," ) ")" ":" (arbno expression)"end")funtion-exp)
    (expression
...
;# 2.6 Definicion de una funcion
      ...
      ;end
       ...
       (funtion-exp (id args exps-body)
               (addGlobalVar id (closure args  exps-body (get-current-env)))
               )
       (return-exp (exp)
                  (begin
...
;
;Esta funcion determina si un elemento es de tipo return-exp para saber si debe retornar algo
(define return-exp?
  (lambda (x)
    (if (expression? x)
        (cases expression x
